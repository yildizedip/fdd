<%@page import="tr.com.fdd.utils.enums.LaboratuvarIslemDurum"%>
...
/* 		SQLUtils sqlUtils = new SQLUtils();
		...
		LaboratuvarIslemDurum durum= LaboratuvarIslemDurum.LABRATUVARDA;
		 
		List<TLabrotuvarDTO> labIslemList = sqlUtils
				.getLabratuvarIslemList(conn, null, durum.getState());
	...
	<hr>
		...
		<hr>
					<form action="labIslemSorgula.do" method="post" id="labrotuvarForm"
						...
						<table class="sorgulama">
							...
							<tr>
								...
								<td>
								<select name="sube">
								<option label="Tümü" value="-1" ></option>
								...
								<option label="${var.toString}" value="${var.state}"
								 <c:if test="${var.state eq 'A'}"> selected="selected" </c:if> />								
								</c:forEach>
								</select></td>
								<td>İşlem</td>
								<td>
								<select name="labIslemTipId" >
								<option label="Tümü" value="-1"></option>
							...
							<tr>
								...
								<td>/</td>
								<td colspan="3"><input class=inputTextfield name="klinikCikisTarBit"
									id="bitTar"></td>
							</tr>

							<tr>
							<td colspan="2" style=" color: red;"> ${warn}</td>
								<td colspan="7" style="text-align: l;"><input
					...
					<table class="sofT" id="example" style="width: 980px">
						<thead>
							<tr>
								<th class="helpHed"></th>
								<th class="helpHed">Şube</th>
								...
								<th class="helpHed">Klinik Değerlendirme Tarihi</th>
								<th class="helpHed">Provolar</th>
								<th class="helpHed"></th>

							</tr>
						</thead>

						<c:forEach items="${labIslemList}" var="labIslem"
							varStatus="count">
							<tr>
								<td class="helpBod">${count.count}</td>
								<td class="helpBod">${labIslem.subeDTO.sAd}</td>
								...
								${labIslem.laboratuvarIslemDurum.toString}
					...
					${prova.provaTipEnm} : ${prova.provaTar}
								...
								<c:choose>
								<c:when test="${labIslem.laboratuvarIslemDurum.state eq 'A'}">
								
								<html:link
										action="/LaboratuvarRandevuVerPageOpen?id=${labIslem.id}"> Prova Randevu Ver </html:link>
							
								</c:when>
								
								<c:when test="${(labIslem.laboratuvarIslemDurum.state eq 'B') || (labIslem.laboratuvarIslemDurum.state eq 'C') || (labIslem.laboratuvarIslemDurum.state eq 'D')}">
								<html:link action="/HastaLabrotuvarIslemleri?labIslemId=${labIslem.id}&labIslemTip=15"> Lab İslem Detay Göster</html:link>
